...
export default {
  ...
  methods: {
    ...
    handleDelete(id) {
      request.delete(`/api/house_need/` + id).then(res => {
        if (res.code === '0') {
    ...
    handleCurneedChange(pageNum) {
      this.curneedPage = pageNum;
      this.load();
    },
    details(row) {
      this.form = row;
    ...
    async handleSubmit() {
      ...
      if (this.form.id) {
        ...
        request.post("/api/house_need", this.form).then(res => {
          console.log(res);
          if (res.code === '0') {
            this.$message({
              type: "success",
